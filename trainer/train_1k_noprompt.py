...
class Trainer():
    ...
    def tr(self):
        ...
        for epoch in range(start,self.epoch):
            ...
            for n_count, data_batch in enumerate(DLoader):
                ...
                batch_img, batch_u0, batch_sino, imgSAM, sr = [x.cuda() for x in data_batch]
                self.model.train()
                self.optimizer.zero_grad()
                batch_x_out, W1, W2, W3, p= self.model(batch_sino, batch_u0, sr)
                lossfunction = torch.nn.MSELoss(reduction='sum')
                loss1 = lossfunction(batch_x_out, batch_img)
                loss2 = self.Myl2_reg_ortho(W1) + self.Myl2_reg_ortho(W2) + self.Myl2_reg_ortho(W3)
                loss = loss1 + (self.args.lamda / (512 * 512)) * loss2
                loss.backward()
                self.optimizer.step()
            ...
            for n_count, data_batch in enumerate(DLoaderval):
                val_img, val_u0, val_sino, imgSAM, sr = [x.cuda() for x in data_batch]
                with torch.no_grad():
                    val_x_db, W1, W2, W3 = self.model(val_sino, val_u0, sr)
    ...
    def aver_ssim(self, img1, img2):
        ...
        SSIM = 0
        img1 = img1.detach().cpu().numpy().astype(np.float32)  #np.float64
        img2 = img2.detach().cpu().numpy().astype(np.float32)  #np.float64
